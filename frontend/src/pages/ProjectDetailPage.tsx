...
export default function ProjectDetailPage() {
  ...
  // Group tasks by service for automated tasks
  const tasksByService: Record<string, any[]> = {}
  const otherTasks: any[] = []
  
  project.tasks?.forEach((task: any) => {
    if (task.isAutomated && task.service) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Tasks Tab - Kanban with Service Grouping */}
      {activeTab === 'tasks' && (
          ...
          {taskColumns.map(status => (
            <div key={status} className="bg-[#09090B] border border-white/10 rounded-2xl p-4">
              ...
              <div className="space-y-2">
                {/* Render automated tasks grouped by service */}
                {Object.entries(tasksByService).map(([serviceName, serviceTasks]) => {
                  const statusTasks = serviceTasks.filter((task: any) => task.status === status)
                  if (statusTasks.length === 0) return null
                  
                  return (
                    <div key={serviceName} className="space-y-2">
                      <div className="text-xs font-bold text-brand-teal mb-1 border-b border-brand-teal/20 pb-0.5">
                        {serviceName}
                      </div>
                      <div className="space-y-2">
                ...
                {/* Render other tasks (non-automated) */}
                {(() => {
                  const otherStatusTasks = otherTasks.filter((task: any) => task.status === status)
                  if (otherStatusTasks.length === 0) return null
                  
                  return (
                    <div className="space-y-2">
                      <div className="text-xs font-bold text-zinc-400 mb-1 border-b border-zinc-700 pb-0.5">
                        Other Tasks
                      </div>
                      <div className="space-y-2">
                        {otherStatusTasks.map((task: any) => (
                          <div key={task.id} className="bg-white/[0.02] border border-white/5 rounded-xl p-3 hover:border-white/10 transition-all group">
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <p className="text-sm text-white font-medium leading-snug flex-1">{task.title}</p>
                              {canManage && (
