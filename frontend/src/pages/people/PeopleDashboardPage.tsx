...
export default function PeopleDashboardPage() {
  ...
  const getTimelineSegments = () => {
    ...
    if (currentMinutes <= lunchStartMinutes) {
      ...
      if (work2Duration > 0) {
        segments.push({
          type: 'working',
          width: `${(work2Duration / totalDuration) * 100}%`,
          label: 'Working'
        })
      }
    } else {
  ...
  const isOvertime = () => {
    const currentMinutes = currentTime.getHours() * 60 + currentTime.getMinutes()
    return currentMinutes > 18.75 * 60 // 6:45 PM
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">HR Dashboard</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { title: 'Total Employees', value: '248', subtext: '+3 this month', icon: Users, color: 'text-brand-teal', bg: 'bg-brand-teal/10' },
          { title: 'Present Today', value: '221', subtext: '89% attendance', icon: UserCheck, color: 'text-brand-mint', bg: 'bg-brand-mint/10' },
          { title: 'Absent Today', value: '18', subtext: '7.2% of total', icon: UserX, color: 'text-red-400', bg: 'bg-red-500/10' },
          { title: 'Late Arrivals', value: '9', subtext: '3.6% of total', icon: Clock, color: 'text-yellow-400', bg: 'bg-yellow-500/10' }
        ].map(({ title, value, subtext, icon: Icon, color, bg }) => (
          <div key={title} className="bg-[#09090B] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-all duration-200">
      ...
      <div className="bg-[#09090B] border border-white/10 rounded-2xl p-5">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={index} className="flex items-center gap-3 p-3 bg-white/[0.02] border border-white/5 rounded-xl hover:border-white/10 transition-all">
