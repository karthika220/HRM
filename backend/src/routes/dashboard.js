...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    console.log('Dashboard stats result:', { activeProjects, completedTasks, overdueTasks, teamMembers, totalProjects, totalTasks, totalUsers });
...
router.get('/task-completion', authenticate, async (req, res) => {
  try {
    ...
    const result = tasksByStatus.map(group => ({
      status: group.status.trim().toUpperCase(),
