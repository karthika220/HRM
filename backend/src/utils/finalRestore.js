...
async function finalRestore() {
  try {
    ...
    // Get a real user for relationships
      ...
      for (let i = 0; i < tasksNeeded; i++) {
        await prisma.task.create({
          data: {
            ...
            assigneeId: owner.id,
            projectId: project.id, // Use project ID directly
            ownerId: owner.id,
            createdAt: new Date(),
            updatedAt: new Date()
          }
    ...
    // Test dashboard
    const dashboardStats = await prisma.task.count();
    const projectCount = await prisma.project.count();
    const userCount = await prisma.user.count();
    
    console.log('✅ Dashboard Data:');
    console.log(`   • Total Projects: ${projectCount}`);
    console.log(`   • Total Tasks: ${dashboardStats}`);
